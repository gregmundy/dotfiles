...
while IFS= read -r line; do
  ...
  status=1
done < <(grep -nE "$PATTERN" install/*.sh || true)
...
while IFS= read -r line; do
  ...
  echo "  $line"
  status=1
done < <(grep -nE '(>>|[^&2]>[[:space:]]*"\$)' install/*.sh | grep -vE '2>&1|>/dev/null|2>/dev/null' || true)

if [[ $status -eq 0 ]]; then
  echo "lint: no unguarded side effects in install/*.sh"
else
